...
import { Check, Star, Zap, CreditCard, History, Sparkles, Loader2, Lock, Smartphone, QrCode, AlertTriangle } from 'lucide-react';
...
const Payments = () => {
    ...
    const [checkoutStep, setCheckoutStep] = useState<'method-selection' | 'payment-details' | 'upi-scan' | 'processing' | 'success'>('method-selection');
    ...
    const [transactions, setTransactions] = useState<any[]>(isPro ? [
        { id: 1, desc: 'Pro Subscription (Monthly)', date: 'Oct 24, 2024', amount: '$4.99', status: 'Paid' }
    ] : []);

    const [paymentMethods, setPaymentMethods] = useState<any[]>(isPro ? [
        ...
        setTimeout(async () => {
            try {
                ...
                const newTx = {
                    ...
                    amount: selectedMethod === 'upi' ? '₹1.00' : (billingCycle === 'monthly' ? '$4.99' : '$49.99'),
                    status: 'Paid'
                };
                setTransactions([newTx, ...transactions]);

                // Add mock method if empty and paid via card
                if (paymentMethods.length === 0 && selectedMethod === 'card') {
                    setPaymentMethods([{ id: Date.now(), type: 'Visa', last4: cardNumber.slice(-4) || '4242', expiry: expiry }]);
                }
    ...
    // Calculate Amount
    const amount = billingCycle === 'monthly' ? 4.99 : 49.99;
        ...
        <Layout>
            <div className="space-y-8 animate-fade-in max-w-5xl mx-auto">
                <div className="text-center space-y-4">
                    <h1 className="text-4xl font-bold tracking-tight">Upgrade Your Productivity</h1>
                    <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                        Get the most out of your day with advanced features, unlimited history, and powerful analytics.
                    </p>
                </div>
                ...
                <Tabs defaultValue="plans" className="space-y-8">
                    ...
                    <TabsContent value="plans" className="space-y-6">
                        <div className="flex justify-center items-center gap-4 mb-8">
                            ...
                            >
                                <div className={`h-4 w-4 rounded-full bg-primary transition-all duration-300 ${billingCycle === 'yearly' ? 'translate-x-6' : ''}`} />
                            </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                            {/* Free Plan */}
                            <Card className={`relative overflow-hidden border-2 transition-all duration-300 ${!isPro ? 'border-primary/50 shadow-lg' : 'border-border'}`}>
                                {!isPro && <div className="absolute top-0 right-0 bg-primary/10 text-primary text-xs font-bold px-3 py-1 rounded-bl-lg">CURRENT PLAN</div>}
                                <CardHeader>
                                    <CardTitle className="text-2xl">Free</CardTitle>
                                    <CardDescription>Essential features for daily tracking</CardDescription>
                                    <div className="mt-4">
                                    ...
                                    <ul className="space-y-3">
                                        {features.map((feature, i) => (
                                            <li key={i} className="flex items-center gap-3">
                                                <div className={`rounded-full p-1 ${feature.free ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}>
                            ...
                            {/* Pro Plan */}
                            <Card className={`relative overflow-hidden border-2 transition-all duration-300 ${isPro ? 'border-primary shadow-lg ring-1 ring-primary/50 scale-[1.02]' : 'border-border hover:border-primary/50'}`}>
                                {isPro && <div className="absolute top-0 right-0 bg-primary text-primary-foreground text-xs font-bold px-3 py-1 rounded-bl-lg">ACTIVE</div>}
                                {!isPro && <div className="absolute -top-10 -right-10 w-32 h-32 bg-primary/5 rounded-full blur-3xl"></div>}
                                ...
                                <CardHeader>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <CardDescription>Power features for serious productivity</CardDescription>
                                        </div>
                                    </div>
                                    <div className="mt-4">
                                        <span className="text-4xl font-bold">{billingCycle === 'monthly' ? '$4.99' : '$49.99'}</span>
                                ...
                                <CardContent>
                                    <ul className="space-y-3">
                                        {features.map((feature, i) => (
                                            <li key={i} className="flex items-center gap-3">
                                                <div className={`rounded-full p-1 ${feature.pro ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
                                                    <Check className="h-3 w-3" />
                                                </div>
                                ...
                                <CardFooter>
                                    {isPro ? (
                                                ...
                                                <div className="py-4 space-y-6">
                                                    <div className="flex items-center justify-between p-4 border rounded-lg bg-muted/20">
                                                        <div className="space-y-1">
                                                            ...
                                                            <p className="text-sm text-muted-foreground">Pro Plan ({billingCycle})</p>
                                                        </div>
                                                        <Badge>Active</Badge>
                                                    </div>
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                    This action cannot be undone. You will lose access to all Pro features immediately.
                                                                </AlertDialogDescription>
                                                            </AlertDialogHeader>
                                                            <AlertDialogFooter>
                                        ...
                                        <Button className="w-full bg-primary hover:bg-primary/90" onClick={handleStartUpgrade}>
                                            Upgrade to Pro
                                        </Button>
                                    )}
                                </CardFooter>
                                        ...
                                        {transactions.map((tx) => (
                                            <div key={tx.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <Badge variant="outline" className="text-green-600 border-green-200 bg-green-50">{tx.status}</Badge>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                    ...
                    <TabsContent value="payment-methods" className="space-y-4 max-w-4xl mx-auto">
                        <Card>
                            <CardHeader>
                                <CardTitle className="flex items-center gap-2">
                    ...
                    <DialogContent className="sm:max-w-md">
                        {checkoutStep === 'method-selection' && (
                            <>
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Choose how you want to pay for Pro ({billingCycle === 'monthly' ? '$4.99/mo' : '$49.99/yr'}).
                                    </DialogDescription>
                                </DialogHeader>
                                <div className="grid gap-4 py-4">
                                    <div
                                        className={`flex items-center gap-4 border rounded-lg p-4 cursor-pointer transition-all ${selectedMethod === 'card' ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'hover:bg-muted'}`}
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="font-medium">UPI / GPay / PayTM</p>
                                            <p className="text-xs text-muted-foreground">Scan QR to pay directly</p>
                                        </div>
                                    </div>
                                </div>
                                <DialogFooter>
                                    <Button variant="outline" onClick={() => setIsCheckoutOpen(false)}>Cancel</Button>
                                    <Button onClick={() => setCheckoutStep(selectedMethod === 'card' ? 'payment-details' : 'upi-scan')}>Continue</Button>
                                </DialogFooter>
                            </>
                        )}
                        ...
                        {checkoutStep === 'payment-details' && (
                            <>
                                ...
                                <div className="grid gap-4 py-4">
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <div className="relative">
                                            <Input id="card" placeholder="0000 0000 0000 0000" value={cardNumber} onChange={e => setCardNumber(e.target.value)} />
                                ...
                                <DialogFooter>
                                    <Button variant="outline" onClick={() => setCheckoutStep('method-selection')}>Back</Button>
                                    <Button onClick={processPayment}>Pay {billingCycle === 'monthly' ? '$4.99' : '$49.99'}</Button>
                                </DialogFooter>
                            </>
                        )}
                            ...
                            <>
                                ...
                                <div className="flex flex-col items-center justify-center py-4 space-y-4">
                                    ...
                                    <div className="text-center">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">Paying: {DEVELOPER_VPA}</p>
                                    </div>
                                ...
                                <DialogFooter>
                                    <Button variant="outline" onClick={() => setCheckoutStep('method-selection')}>Back</Button>
                                    <Button onClick={processPayment} className="gap-2 bg-green-600 hover:bg-green-700">
                        ...
                        {checkoutStep === 'processing' && (
                            <div className="flex flex-col items-center justify-center py-8 space-y-4">
                                ...
                                <div className="text-center space-y-1">
                                    ...
                                    <p className="text-sm text-muted-foreground animate-pulse">Checking status with bank server...</p>
                                </div>
                            </div>
                        )}
                        ...
                        {checkoutStep === 'success' && (
                            <div className="flex flex-col items-center justify-center py-6 space-y-4 text-center">
                                ...
                                <p className="text-muted-foreground px-4">
                                    Thank you for your purchase. Your account has been instantly upgraded to Pro.
                                </p>
