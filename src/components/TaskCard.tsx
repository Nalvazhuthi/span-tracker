...
import { formatDisplayDate, formatShortDate, getTotalDays, getToday, getDaysInRange } from '@/utils/dateUtils';
...
const priorityBadge: Record<string, { bg: string; text: string }> = {
  ...
  medium: { bg: 'bg-amber-50 dark:bg-amber-900/20', text: 'text-amber-600 dark:text-amber-400' },
...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onEdit, onDelete, compact = false }) => {
  ...
  const allDays = getDaysInRange(task.startDate, task.endDate);
  const activeDays = getActiveTaskDays(task, allDays);
  const totalActive = activeDays.length;
  const passedDays = activeDays.filter(d => d <= today).length;
  const timePercentage = totalActive > 0 ? Math.round((passedDays / totalActive) * 100) : 0;
  ...
  if (compact) {
    return (
      <div className="group relative flex items-center gap-4 rounded-xl border border-border bg-card p-3 hover:bg-muted/30 transition-all duration-200">
        {/* Status Strip */}
        <div className={cn('absolute left-0 top-3 bottom-3 w-1 rounded-r-full', categoryColors[task.category])} />
        
        <div className="pl-3 flex-1 min-w-0 grid grid-cols-[1fr_auto] gap-4 items-center">
          <div className="min-w-0">
             <div className="flex items-center gap-2 mb-1">
               <span className="font-semibold text-foreground truncate text-sm">{task.name}</span>
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex flex-col gap-1.5">
           <div className="flex items-center gap-2">
              <span className={cn('px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider', priorityBadge[priority].bg, priorityBadge[priority].text)}>
                {priority} Priority
              </span>
              {isActive && (
           ...
           <div className="flex items-center gap-2">
             <div className={cn('h-2 w-2 rounded-full', categoryColors[task.category])} />
             <span className="text-xs text-muted-foreground font-medium">{categoryLabel}</span>
           </div>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-end justify-between text-sm">
          <div className="flex flex-col">
             <span className="text-xs text-muted-foreground font-medium mb-0.5">Completion</span>
             <span className="font-bold text-xl">{progress.percentage}%</span>
          </div>
          <div className="text-xs text-muted-foreground text-right">
             <span className="font-medium text-foreground">{progress.completed}</span> / {progress.total} days
          </div>
        </div>
